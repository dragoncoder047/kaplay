...
onUpdate(() => {
    t.text = Object.entries(tests).map(([name, { n, p }]) => `${name}: ${p}/${n} ${(p / n * 100).toFixed(1)}%`).join("\n")
})
...
test("random inout", () => {
    ...
    m1.setTRSS(translation.x, translation.y, rotation, scale.x, scale.y, skew.x, skew.y);
    const translation2 = m1.getTranslation();
    const rotation2 = m1.getRotation();
    const scale2 = m1.getScale();
    const skew2 = m1.getSkew();
    m2.setTRSS(translation2.x, translation2.y, rotation2, scale2.x, scale2.y, skew2.x, skew2.y);
    debug.clearLog();
    debug.log("\n\n");
    debug.log("translation", translation, translation2, "error", translation2.sub(translation));
...
test("fully random matrix", () => {
    ...
    m2.setTRSS(translation.x, translation.y, rotation, scale.x, scale.y, skew.x, skew.y);
    return (
        similar(m1.a, m2.a) &&
        similar(m1.b, m2.b) &&
        similar(m1.c, m2.c) &&
